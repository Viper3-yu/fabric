...
MySQL 遥测（网点、路线计划、GPS、温湿度）为可选数据源，表结构在 `deploy/mysql/`。配置 `MYSQL_DSN` 后控制塔读取真实遥测；未配置时使用内置演示路线并在响应中标注 `dataSource: demo`。**已配置数据源时查不到数据会直接报错，不会静默回退演示数据**（2026-09-05 审计结论，见 `docs/AUDIT-2026-09-05.md`）。
