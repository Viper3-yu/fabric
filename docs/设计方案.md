...
项目采用 B/S 架构。浏览器负责业务操作和公开查询，Node.js 服务负责鉴权、参数校验和 Fabric Gateway 调用，Hyperledger Fabric 智能合约负责状态流转、组织权限、关键证据哈希和不可篡改历史。
...
| 课程要求                    | 设计响应                                                 | 验收证据                                   |
| --------------------------- | -------------------------------------------------------- | ------------------------------------------ |
| B/S 应用                    | React 浏览器端 + Express API                             | 本机浏览器可访问，接口可独立测试           |
...
| 界面美观                    | IBM Carbon 组件体系，自定义冷灰与翡翠色品牌令牌          | 桌面和移动端浏览器视觉验收                 |
...
- 后端：Node.js、Express、TypeScript、Zod、JWT、Hyperledger Fabric Gateway SDK。
- 智能合约：TypeScript、`fabric-contract-api`、`fabric-shim`。
...
界面规则：使用一个翡翠色强调色，卡片 12px 圆角，按钮 8px 圆角，数字和交易 ID 使用等宽字体。状态颜色只表达真实语义，不作为装饰。支持 360px 以上移动端、键盘导航、可见焦点、骨架加载、空状态和错误恢复。

## 10. 安全与可靠性

- API 使用 Zod 拒绝非法状态和超长字段。
- JWT 设置有效期，密钥只通过环境变量提供。
...
当前执行环境未安装 Docker，因此可以完成源码、单元测试、演示闭环和浏览器验收，但真实 Fabric 容器网络的最终启动验证需要在安装 Docker Desktop 的机器上补做。此限制不改变代码必须包含完整 Fabric 接入路径的要求。
