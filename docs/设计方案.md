...
本项目不把普通数据库记录包装成区块链结果。每次建单、接单、揽收、轨迹更新、异常上报、送达和签收都必须提交 Fabric 交易，并返回交易 ID。API 只连接真实 Fabric 网络；单元与闭环测试使用文件型测试替身模拟链上状态机，不会把测试结果当作真实上链。
...
控制塔场景在运单聚合之外新增三类实体，均独立上键存储，并通过 `SHIPMENTPARCEL:` / `SHIPMENTUNIT:` / `SHIPMENTHANDOVER:` 前缀键与运单关联：

| 实体            | 关键字段                                                                                                   | 说明                                                  |
| --------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `Parcel`        | `id`、`shipmentId`、`status`（CREATED/PACKED/UNPACKED）、`unitId`                                          | 一个物理包裹；装箱后记录所属物流单元                  |
| `LogisticsUnit` | `id`、`shipmentId`、`unitType`、`parcelIds`、`status`（PACKED/UNPACKED）                                   | 多个包裹的运输聚合（托盘/箱），拆箱后包裹状态同步更新 |
| `Handover`      | `handoverId`、`shipmentId`、`fromHub`、`toHub`、`carrierOrg`、`status`（INITIATED/CONFIRMED）、双方交易 ID | 跨组织责任交接，发起与确认是两笔独立交易              |
...
控制塔协作交易（与运单状态机解耦，不改变运单状态）：

| 交易                                       | 调用方       | 前置状态                     | 结果                                                           |
| ------------------------------------------ | ------------ | ---------------------------- | -------------------------------------------------------------- |
| `AddParcel`                                | Org1MSP      | `CREATED/ACCEPTED`           | 登记包裹                                                       |
| `PackParcels`                              | Org1MSP      | `CREATED/ACCEPTED`，包裹待装 | 建物流单元，包裹进入 `PACKED`                                  |
| `UnpackParcels`                            | Org2MSP      | 单元 `PACKED`                | 单元与包裹进入 `UNPACKED`                                      |
| `InitiateHandover`                         | Org1MSP      | `ACCEPTED` 起的在途状态      | 创建 `INITIATED` 交接单                                        |
| `ConfirmHandover`                          | Org2MSP      | 交接单 `INITIATED`           | 进入 `CONFIRMED`；发起方无法自确认，不可重复确认               |
| `RecordNodeEvent`                          | 双组织       | 运单在途                     | 追加通用事件，**不改变状态**；`actorOrg` 必须与调用方 MSP 一致 |
| `GetParcels` / `GetUnits` / `GetHandovers` | 任意通道成员 | 运单存在                     | 按运单返回对应实体集合                                         |
...
| 公开查询 | 无需登录，运单号查询、脱敏轨迹、账本模式标记         |
| 验真结果 | 哈希匹配结果、历史连续性、最终状态和风险提示         |

界面规则：使用一个森林绿强调色，业务表面 8px 圆角，控件 4px 圆角，状态标签使用胶囊几何，数字和交易 ID 使用等宽字体。状态颜色只表达真实语义，不作为装饰。公开叙事标题使用 `Noto Serif SC Variable` 衬线体形成档案编辑感，正文与后台使用 `Manrope Variable` + `Noto Sans SC Variable`。支持 360px 以上移动端、键盘导航、可见焦点、骨架加载、空状态和错误恢复。视觉规范详见 `docs/设计系统.md`。
...
- 账号密码不由源代码提供：开发和部署均通过 `APP_PASSWORD_<账号>` 或 `APP_PASSWORD_HASH_<账号>` 环境变量配置（生产环境必须配置其一），哈希可用 `go run ./apps/api/cmd/hash-password` 生成。
- 服务端角色校验与链码 MSP 校验双重执行。
- 交易时间来自 Fabric，不信任浏览器时间。
- 签收码使用 SHA-256 摘要，链上不保存明文。
- 同一动作在错误状态下重复调用会失败，避免重复揽收和重复签收。
- 公开接口只返回脱敏数据，并对查询频率预留限流配置。
- 所有修改响应携带 `ledgerMode=fabric` 与真实交易 ID，页面明确展示账本模式，避免把模拟结果误认为真实上链。

## 11. 运行方案

### 本地运行

按 README 完成 `fabric:bootstrap` 与 `fabric:up` 后启动 API 与前端，所有业务请求通过 Fabric Gateway 提交真实交易，页面展示的运单与事件全部来自链上账本。

### 自动化测试

单元与闭环测试使用文件型测试替身（`apps/api/internal/ledger/fake`）模拟链上状态机，不需要 Docker；`pnpm test:fabric` 在运行中的测试网络上执行真实的建单到签收闭环。
...
- README 提供从零启动、账号配置、常见故障和提交目录说明。

真实 Fabric 网络验收以 `pnpm test:fabric` 在运行中的测试网络上通过为准；该结论仅适用于本机测试网络，不外推为生产网络验收。
